...
const STORAGE = 'taskflow-tasks-v3';
...
const starterTasks = [
  ...
  { id: '5', title: 'Review notes', category: 'Study', priority: 'Medium', due: 'Today', time: '20:00', repeat: 'Every day', done: false, lastDoneDate: null },
];
...
function App() {
  ...
  const completed = tasks.filter(t => t.repeat === 'None' ? t.done : recurringDoneToday(t)).length;
  const pending = todayTasks.filter(t => !active(t)).length;
  const progress = todayTasks.length ? Math.round((todayTasks.filter(active).length / todayTasks.length) * 100) : 0;
  ...
  const exportData = () => { const blob = new Blob([JSON.stringify({ version: 3, exportedAt: new Date().toISOString(), tasks }, null, 2)], { type: 'application/json' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `taskflow-backup-${TODAY()}.json`; a.click(); URL.revokeObjectURL(a.href); setNotice('Backup exported.'); };
  const importData = e => { const file = e.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => { try { const data = JSON.parse(reader.result); if (!Array.isArray(data.tasks)) throw new Error(); setTasks(data.tasks); setNotice('Backup restored successfully.'); } catch { setNotice('Invalid TaskFlow backup file.'); } }; reader.readAsText(file); e.target.value = ''; };

  return <div className="app-shell"><div className="glow glow-one"/><div className="glow glow-two"/><main className="phone">
    <header className="topbar"><div><span className="eyebrow"><Sparkles size={13}/> TASKFLOW</span><h1>Good evening, Maha<span>.</span></h1><p>{new Intl.DateTimeFormat('en-IN',{weekday:'long',month:'long',day:'numeric'}).format(new Date())} · Keep your momentum going.</p></div><button className="icon-btn" onClick={() => setDark(v => !v)}>{dark ? <Sun size={19}/> : <Moon size={19}/>}</button></header>

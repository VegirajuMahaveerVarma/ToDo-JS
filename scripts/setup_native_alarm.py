...
public class TaskflowAlarmPlugin extends Plugin {
    ...
    private void ensureChannel(Context context) {
        ...
        NotificationChannel channel = new NotificationChannel(
                CHANNEL_ID, "TaskFlow Alarms", NotificationManager.IMPORTANCE_HIGH
        );
...
if 'TaskflowAlarmReceiver' not in s:
    s = s.replace(
        '</application>',
        '        <receiver android:name=".TaskflowAlarmReceiver" android:exported="false" />\n'
        '    </application>',
        1
    )
manifest.write_text(s, encoding='utf-8')

# Make the web app call the native exact-alarm bridge in Android builds.
...
if "registerPlugin('TaskflowAlarm')" not in s:
    s = s.replace(
        ...
        "const TaskflowAlarm = registerPlugin('TaskflowAlarm');",
        1
    )
